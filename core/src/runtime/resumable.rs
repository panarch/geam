...
pub(crate) use callback::{AsyncHostCallbackRequest, CallbackRequest, ResumableCallback};
pub(crate) use error::TransferExecutionError;
...
macro_rules! run_embedded_scalar {
    ($name:ident, $run:ident, $function:ty, $return:ty, $map:expr) => {
        ...
        ) -> Result<$return, crate::ExecutionError>
        where
            ...
            Profile::ExternalStores: Send,
        {
            ...
            .await
            .map($map)
            .map_err(TransferExecutionError::into_execution)
        }
    };
}
...
pub(crate) async fn run_embedded_custom<Profile>(
    ...
    host: &mut Profile::RunState,
    echo: &mut (dyn EchoSink + Send),
) -> Result<crate::runtime::EmbeddingOutput<TransferValues>, crate::ExecutionError>
where
    Profile: crate::HostProfile,
    Profile::RunState: Send,
    Profile::ExternalStores: Send,
...
"#,
        ...
        let (execution, entries) =
            AsyncHostedExecution::from_library_plan(plan, retain_entry, vec![stop_entry]);
        ...
        (execution, retain_function, stop_function, type_id)
    }
    ...
    fn external_payload_survives_pending_and_releases_on_completion_or_cancellation() {
        let (execution, function, _, type_id) = external_execution();
    ...
    fn non_returning_host_specialized_to_an_external_return_preserves_its_failure() {
        let (execution, _, function, _) = external_execution();
        let mut state = ();
        let mut stores = ();
        let mut echo = RecordedEcho::default();
        let wake = Arc::new(WakeFlag(AtomicBool::new(false)));
