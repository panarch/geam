...
use gleam_compiler_core::build::{Origin, Target};
use gleam_compiler_core::config::PackageConfig;
use gleam_compiler_core::line_numbers::LineNumbers;
use gleam_compiler_core::parse;
...
use gleam_compiler_core::warning::{TypeWarningEmitter, WarningEmitter};
...
fn host_type(type_: &HostTypeDescriptor) -> std::sync::Arc<gleam_compiler_core::type_::Type> {
    match type_ {
...
fn host_function_type(
    arguments: &[HostTypeDescriptor],
    return_: &HostTypeDescriptor,
) -> std::sync::Arc<gleam_compiler_core::type_::Type> {
    fn_(
        arguments.iter().map(host_type).collect(),
        host_type(return_),
    )
...
mod tests {
    ...
    use ecow::EcoString;
    use gleam_compiler_core::ast::{Publicity, SrcSpan};
    use gleam_compiler_core::type_::error::VariableOrigin;
    use gleam_compiler_core::type_::{
...
"#,
    ...
    fn leaves_undeclared_and_unknown_host_imports_to_gleam_analysis() {
        ...
        assert!(matches!(
            ...
            FrontendError::Analyse { errors }
                if matches!(
                    errors.as_slice(),
                    [gleam_compiler_core::type_::Error::UnknownModule { name, .. }]
                        if name == "host/math"
                )
        ));
        assert!(matches!(
    ...
    fn reject_margin_hosted_typed_program_invalid_constant_shape() {
        ...
        source_module.module.definitions.constants[0].type_ =
