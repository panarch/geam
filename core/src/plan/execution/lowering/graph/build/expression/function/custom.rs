...
pub(in crate::plan::execution::lowering) fn custom_function_expr(
    ...
    let shape = context.concrete_function_shape(&expression.custom_function_type().shape());
    ...
    match kind {
        ...
        E::Call {
            ...
            DraftFlow::Value {
                ...
                .map(|function| {
                    let function = execution::function::FunctionFunctionId::Custom(function);
                    let value = graph.function_instruction(
                        &mut cursor,
                        shape.clone(),
                        I::Call {
        ...
        super::generic::executable_kind_lowering(
            ...
            |function, context| {
                ...
                context
                    ...
                    .map(execution::function::FunctionFunctionId::Custom)
            },
            |branch, cursor, graph, context| {
                generic_custom_function_expr_kind(
                    branch,
