...
fn lower_step(
    ...
    match step.kind() {
        ...
        S::BindCustomFields { local, pattern } => {
            let source = cursor.scope().custom(super::local::LocalKey::new(
...
fn assert_subject(subject: &module::AssertSubject, cursor: &DraftCursor) -> DraftValueRef {
    ...
    match subject {
        module::AssertSubject::Int(local) => scope
            .int(super::local::LocalKey::new(
                super::local::LocalKind::Int,
                local.0,
            ))
            .erase(),
        module::AssertSubject::Float(local) => scope
            .float(super::local::LocalKey::new(
                super::local::LocalKind::Float,
                local.0,
            ))
            .erase(),
        module::AssertSubject::String(local) => scope
            .string(super::local::LocalKey::new(
                super::local::LocalKind::String,
                local.0,
            ))
            .erase(),
        module::AssertSubject::BitArray(local) => scope
            .bit_array(super::local::LocalKey::new(
                super::local::LocalKind::BitArray,
                local.0,
            ))
            .erase(),
        module::AssertSubject::Custom(local) => scope
            .custom(super::local::LocalKey::new(
                super::local::LocalKind::Custom,
                local.id().0,
            ))
            .erase(),
        module::AssertSubject::Bool(local) => scope
            .bool(super::local::LocalKey::new(
                super::local::LocalKind::Bool,
                local.0,
            ))
            .erase(),
        ...
        module::AssertSubject::List(local) => {
            scope.list(super::local::list_local_key(local)).erase()
        }
    }
}
...
struct MatchPlanner<'a> {
    ...
    binding_indices: HashMap<super::local::LocalKey, usize>,
    graph: &'a mut DraftGraph,
    context: &'a mut super::LoweringContext,
}
...
impl<'a> MatchPlanner<'a> {
    ...
    fn assert_binding(
        ...
        self.context
            ...
            .map(|shape| self.bind(super::local::param_local_key(binding.slot().local()), shape))
    }
    ...
    fn bit_array_segment(
        ...
        match segment {
            ...
            S::UtfCodepoint { pattern, encoding } => DraftBitArrayPatternSegment::UtfCodepoint {
                pattern: self.bit_array_binding(
                    pattern,
                    super::local::LocalKind::UtfCodepoint,
        ...
        match pattern {
            module::BitArrayBindingPattern::Bind(binding) => {
                DraftBitArrayBindingPattern::Bind(self.bind(
                    super::local::LocalKey::new(kind, binding.local().index()),
                    shape,
                ))
            }
    ...
    fn bit_array_size_expr(
        ...
        match value {
            ...
            E::LocalGet { local, .. } => {
                let key = super::local::LocalKey::new(super::local::LocalKind::Int, local.0);
    ...
    match pattern.kind() {
        P::Bind(binding) => stored_pattern_shape(pattern, context).map(|_| {
            cursor.scope_mut().insert(
                super::local::param_local_key(binding.slot().local()),
                source,
            );
            cursor
        }),
