...
pub(super) fn capture_args(
    ...
    for (index, arg) in args.iter().enumerate() {
        let source = cursor
            .scope()
            .get(super::local::param_local_key(arg.local()));
        let target = context.target_capture_local(
            function,
            module::CapturePosition::new(index),
            source.shape().clone(),
        );
        let target = super::local::stored_value_local_at(target.shape(), target.index(), context);
...
pub(super) fn bool_case<Value>(
    ...
    cursor: DraftCursor,
    case: CaseLowering<'_>,
    lower_true: impl FnOnce(DraftCursor, &mut DraftGraph, &mut super::LoweringContext) -> Lowered<Value>,
