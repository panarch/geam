...
The prepared path uses `src/geam_bindings/program.rs`, generated by sync and
ignored by Git, and fresh native implementations. The dynamic path additionally
reads the local Gleam project. `cargo run --quiet --locked -- --prepared` runs only the prepared path,
