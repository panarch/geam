...
impl Preparation {
    ...
    fn generate_in(
        ...
        let artifact = match self.artifact {
            Artifact::Plain => "data::ModuleArtifact<std::convert::Infallible>",
            Artifact::Hosted => "data::HostedModuleArtifact",
        };
        Ok(format!(
            "{}\n// Preparation inputs: {}\nuse {}::__prepared_support as data;\n\n#[rustfmt::skip]\npub(super) static PROGRAM: {artifact} = {expression};\n",
...
mod tests {
    ...
    fn preserves_preparation_workspace_and_cargo_failures() {
        ...
        let input = generated.lines().nth(2).unwrap();
        assert!(input.starts_with("// Preparation inputs: "));
        assert_eq!(
            generated.replace(input, "// Preparation inputs: <inputs>"),
