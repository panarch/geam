...
pub(crate) struct SomeDefinition;

pub(crate) struct NoneDefinition;

pub(crate) struct SomeField;

type Arguments<Value> = HostTypeList<Value, HostTypeListEnd>;
pub(crate) type GleamOption<Value> = HostCustomType<OptionSchema, Arguments<Value>>;
pub(crate) type GleamSome<Value> =
    HostCustomConstructorAt<GleamOption<Value>, HostCustomIndex0, SomeDefinition>;
pub(crate) type GleamNone<Value> = HostCustomConstructorAt<
    GleamOption<Value>,
