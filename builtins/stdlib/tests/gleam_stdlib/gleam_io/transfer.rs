...
use geam_core::{EchoOutput, EchoSink, HostProfile, HostProviderSet};
use geam_stdlib::{
    Component, GleamStdlibHostProfile, GleamStdlibRunState, GleamStdlibStores, IoOutput, IoSink,
    IoStream,
};
use std::cell::Cell;
...
fn preserves_io_echo_and_failure_with_a_send_only_caller_sink() {
    ...
    let program = observed_project(
        ...
        HostProviderSet::from_providers(
            geam_stdlib::host_providers::<Profile>().expect("IO transfer registration"),
        )
    ...
    let geam_core::embedding::CallError::Execution(geam_core::ExecutionError::Panic(panic)) = error
    else {
...
impl HostComponentProfile<Component<Sink>> for Profile {
