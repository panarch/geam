...
pub fn main() {
  ...
  assert decode.run(dynamic.bit_array(<<1, 2>>), decode.bit_array)
    == Ok(<<1, 2>>)
  assert decode.run(dynamic.int(42), decode.float)
    == Error([
      decode.DecodeError(expected: "Float", found: "Int", path: []),
    ])
  assert decode.run(dynamic.float(42.0), decode.int)
    == Error([
      decode.DecodeError(expected: "Int", found: "Float", path: []),
    ])
  assert decode.run(dynamic.bit_array(<<"Ada":utf8>>), decode.string)
    == Ok("Ada")
  assert decode.run(dynamic.bit_array(<<1:size(1)>>), decode.string)
    == Error([
