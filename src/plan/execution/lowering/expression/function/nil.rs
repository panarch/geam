...
pub(in crate::plan::execution::lowering) fn nil_function_expr(
    expression: &module::NilFunctionExpr,
    context: &mut super::super::super::LoweringContext,
) -> Representability<execution::NilFunctionExpr> {
    use execution::NilFunctionExprKind as E;
    use module::NilFunctionExprKind as M;

    let kind = match expression.kind() {
        M::Constant(value) => context.nil_function_constant(value).map(E::Constant),
        M::Reference(value) => {
            super::function_reference(value, context, |id, context| context.nil_function_id(id))
                .map(E::Reference)
        }
        M::Closure {
            function,
            ...
            context.nil_function_id(id)
        })
        .map(E::Closure),
        M::LocalGet { local, name: _ } => Representability::Inhabited(E::LocalGet {
            local: execution::NilFunctionLocalId(
                context.mapped_local(super::super::super::frame::LocalKind::NilFunction, local.0),
            ),
        }),
        M::Call {
            function,
        ...
        M::BoolCase {
            ...
            subject,
            context,
            |context| nil_function_expr(true_, context),
            |context| nil_function_expr(false_, context),
            execution::NilFunctionExpr::into_kind,
        ...
        M::IntCase {
            ...
            Representability::collect(clauses.iter().map(|(pattern, branch)| {
                nil_function_expr(branch, context).map(|branch| (pattern.clone(), branch))
            }))
            .and_then(|clauses| {
                nil_function_expr(fallback, context).map(|fallback| E::IntCase {
                    subject: Box::new(subject),
                    clauses,
        ...
        M::StringCase {
            ...
            .and_then(|clauses| {
                nil_function_expr(fallback, context).map(|fallback| E::StringCase {
                    subject: Box::new(subject),
                    clauses,
                    fallback: Box::new(fallback),
                })
        ...
        M::FloatCase {
            ...
            Representability::collect(clauses.iter().map(|(pattern, branch)| {
                nil_function_expr(branch, context).map(|branch| (*pattern, branch))
            }))
            .and_then(|clauses| {
                nil_function_expr(fallback, context).map(|fallback| E::FloatCase {
                    subject: Box::new(subject),
                    clauses,
                    fallback: Box::new(fallback),
                })
        ...
        M::Block { steps, return_ } => {
            crate::plan::execution::lowering::step::steps(steps, context).and_then(|steps| {
                nil_function_expr(return_, context).map(|return_| E::Block {
                    steps,
    ...
    kind.map(execution::NilFunctionExpr::from_kind)
}
