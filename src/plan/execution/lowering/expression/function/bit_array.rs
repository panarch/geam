...
pub(in crate::plan::execution::lowering) fn bit_array_function_expr(
...
) -> Representability<execution::BitArrayFunctionExpr> {
    use execution::BitArrayFunctionExprKind as E;
    use module::BitArrayFunctionExprKind as M;

    let kind = match expression.kind() {
        M::Constant(value) => context.bit_array_function_constant(value).map(E::Constant),
        ...
        M::BoolCase {
            ...
            subject,
            context,
            |context| bit_array_function_expr(true_, context),
            |context| bit_array_function_expr(false_, context),
            execution::BitArrayFunctionExpr::into_kind,
        ...
        M::IntCase {
            ...
            Representability::collect(clauses.iter().map(|(pattern, branch)| {
                bit_array_function_expr(branch, context).map(|branch| (pattern.clone(), branch))
            }))
            .and_then(|clauses| {
                bit_array_function_expr(fallback, context).map(|fallback| E::IntCase {
                    subject: Box::new(subject),
                    clauses,
        ...
        M::StringCase {
            ...
            .and_then(|clauses| {
                bit_array_function_expr(fallback, context).map(|fallback| E::StringCase {
                    subject: Box::new(subject),
                    clauses,
                    fallback: Box::new(fallback),
                })
        ...
        M::FloatCase {
            ...
            Representability::collect(clauses.iter().map(|(pattern, branch)| {
                bit_array_function_expr(branch, context).map(|branch| (*pattern, branch))
            }))
            .and_then(|clauses| {
                bit_array_function_expr(fallback, context).map(|fallback| E::FloatCase {
                    subject: Box::new(subject),
                    clauses,
                    fallback: Box::new(fallback),
                })
        ...
        M::Block { steps, return_ } => {
            crate::plan::execution::lowering::step::steps(steps, context).and_then(|steps| {
                bit_array_function_expr(return_, context).map(|return_| E::Block {
                    steps,
    ...
    kind.map(execution::BitArrayFunctionExpr::from_kind)
}
