...
pub(in crate::plan::execution::lowering) fn string_function_expr(
    ...
    let kind = match expression.kind() {
        ...
        M::Closure {
            ...
            captures,
            context,
            |id, context| context.string_function_id(id),
        )),
        ...
        M::Call {
            ...
            function: context.string_function_function_id(function),
        ...
        M::BoolCase {
            ...
            true_: Box::new(string_function_expr(true_, context)),
            false_: Box::new(string_function_expr(false_, context)),
        },
        M::IntCase {
            subject,
            clauses,
            ...
            clauses: clauses
                .iter()
                .map(|(pattern, branch)| (pattern.clone(), string_function_expr(branch, context)))
                .collect(),
            fallback: Box::new(string_function_expr(fallback, context)),
        },
        M::StringCase {
            subject,
            clauses,
        ...
        M::FloatCase {
            ...
            clauses: clauses
                .iter()
                .map(|(pattern, branch)| (*pattern, string_function_expr(branch, context)))
                .collect(),
            fallback: Box::new(string_function_expr(fallback, context)),
        },
        M::Block { steps, return_ } => E::Block {
            steps: crate::plan::execution::lowering::step::steps(steps, context),
            return_: Box::new(string_function_expr(return_, context)),
        },
    };

    execution::StringFunctionExpr::from_kind(kind)
}
