...
pub(in crate::plan::execution::lowering) fn bool_expr(
    ...
    execution::BoolExpr::from_kind(match expression.kind() {
        ...
        M::Call { function, args } => E::Call {
            function: context.bool_function_id(function),
        ...
        M::BitArrayMatches { value, pattern } => E::BitArrayMatches {
            ...
            pattern: super::super::pattern::bit_array_pattern(pattern, context),
        },
        ...
        M::IntCase {
            ...
            clauses: clauses
                .iter()
                .map(|(pattern, branch)| (pattern.clone(), bool_expr(branch, context)))
                .collect(),
