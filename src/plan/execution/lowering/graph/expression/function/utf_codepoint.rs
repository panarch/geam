...
pub(in crate::plan::execution::lowering) fn utf_codepoint_function_expr(
...
) -> Representability<DraftFlow<DraftUtfCodepointFunction>> {
    use super::super::super::instruction::DraftFunctionInstruction as I;
    use module::UtfCodepointFunctionExprKind as E;
    ...
    match expression.kind() {
        E::Constant(value) => context.utf_codepoint_function_constant(value).map(|id| {
        ...
        E::Reference(value) => context
            .utf_codepoint_function_id(value.instantiation())
            .map(|target| {
                reference(
                    shape.clone(),
                ...
                .map(DraftUtfCodepointFunction::new)
            }),
        E::Closure { function, captures } => {
            context
                .utf_codepoint_function_id(function)
                .and_then(|target| {
                    closure(
                        function,
                        captures,
                    ...
                    .map(|flow| flow.map(DraftUtfCodepointFunction::new))
                })
        }
        E::LocalGet { local, name: _ } => {
            let value = cursor
                ...
                .function(super::super::super::super::local::LocalKey::new(
                    super::super::super::super::local::LocalKind::UtfCodepointFunction,
                    local.0,
                ));
            Representability::Inhabited(DraftFlow::value(
                cursor,
                DraftUtfCodepointFunction::new(value),
            ))
        }
        E::Call {
            function,
        ...
        E::BoolCase {
            ...
            |cursor, graph, context| utf_codepoint_function_expr(true_, cursor, graph, context),
            |cursor, graph, context| utf_codepoint_function_expr(false_, cursor, graph, context),
            DraftUtfCodepointFunction::from_ref,
        ),
        E::IntCase {
            subject,
            clauses,
            ...
            |branch, cursor, graph, context| {
                utf_codepoint_function_expr(branch, cursor, graph, context)
            },
            DraftUtfCodepointFunction::from_ref,
        ),
        E::StringCase {
            ...
            super::super::super::step::steps(steps, cursor, graph, context).and_then(|flow| {
                match flow {
                    ...
                    DraftFlow::Value { cursor, value: () } => {
                        utf_codepoint_function_expr(return_, cursor, graph, context)
                    }
                }
            })
        }
