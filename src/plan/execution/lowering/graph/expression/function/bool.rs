...
pub(in crate::plan::execution::lowering) fn bool_function_expr(
...
) -> Representability<DraftFlow<DraftBoolFunction>> {
    use super::super::super::instruction::DraftFunctionInstruction as I;
    use module::BoolFunctionExprKind as E;
    ...
    match expression.kind() {
        E::Constant(value) => context.bool_function_constant(value).map(|id| {
        ...
        E::Closure { function, captures } => {
            context.bool_function_id(function).and_then(|target| {
                ...
                .map(|flow| flow.map(DraftBoolFunction::new))
            })
        }
        E::LocalGet { local, name: _ } => {
            let value = cursor
                ...
                .function(super::super::super::super::local::LocalKey::new(
                    super::super::super::super::local::LocalKind::BoolFunction,
                    local.0,
                ));
            Representability::Inhabited(DraftFlow::value(cursor, DraftBoolFunction::new(value)))
        }
        E::Call {
            function,
            args,
        ...
        E::Panic(value) => {
            source_stop(value, cursor, graph, context).map(|flow| flow.map(DraftBoolFunction::new))
        }
        E::BoolCase {
            subject,
            true_,
            ...
            |cursor, graph, context| bool_function_expr(true_, cursor, graph, context),
            |cursor, graph, context| bool_function_expr(false_, cursor, graph, context),
            DraftBoolFunction::from_ref,
        ),
        E::IntCase {
            subject,
            clauses,
            ...
            super::super::super::step::steps(steps, cursor, graph, context).and_then(|flow| {
                match flow {
                    ...
                    DraftFlow::Value { cursor, value: () } => {
                        bool_function_expr(return_, cursor, graph, context)
                    }
                }
            })
        }
