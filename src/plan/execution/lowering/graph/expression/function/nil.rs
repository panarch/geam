...
pub(in crate::plan::execution::lowering) fn nil_function_expr(
...
) -> Representability<DraftFlow<DraftNilFunction>> {
    use super::super::super::instruction::DraftFunctionInstruction as I;
    use module::NilFunctionExprKind as E;
    ...
    match expression.kind() {
        E::Constant(value) => context.nil_function_constant(value).map(|id| {
        ...
        E::Closure { function, captures } => context.nil_function_id(function).and_then(|target| {
            ...
            .map(|flow| flow.map(DraftNilFunction::new))
        }),
        E::LocalGet { local, name: _ } => {
            let value = cursor
                .scope()
                .function(super::super::super::super::local::LocalKey::new(
                    super::super::super::super::local::LocalKind::NilFunction,
                    local.0,
                ));
            Representability::Inhabited(DraftFlow::value(cursor, DraftNilFunction::new(value)))
        }
        E::Call {
            function,
            args,
        ...
        E::Panic(value) => {
            source_stop(value, cursor, graph, context).map(|flow| flow.map(DraftNilFunction::new))
        }
        E::BoolCase {
            subject,
            true_,
            ...
            |cursor, graph, context| nil_function_expr(true_, cursor, graph, context),
            |cursor, graph, context| nil_function_expr(false_, cursor, graph, context),
            DraftNilFunction::from_ref,
        ),
        E::IntCase {
            subject,
            clauses,
            ...
            super::super::super::step::steps(steps, cursor, graph, context).and_then(|flow| {
                match flow {
                    ...
                    DraftFlow::Value { cursor, value: () } => {
                        nil_function_expr(return_, cursor, graph, context)
                    }
                }
            })
        }
