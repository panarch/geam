...
pub(in crate::plan::execution::lowering) fn tuple_function_expr(
...
) -> Representability<DraftFlow<DraftTupleFunction>> {
    use super::super::super::instruction::DraftFunctionInstruction as I;
    use module::TupleFunctionExprKind as E;
    ...
    match expression.kind() {
        E::Constant(value) => context.tuple_function_constant(value).map(|id| {
        ...
        E::Closure {
            ...
            captures,
            return_type: _,
        } => context.tuple_function_id(function).and_then(|target| {
            closure(
                function,
                captures,
                shape.clone(),
            ...
            .map(|flow| flow.map(DraftTupleFunction::new))
        }),
        E::LocalGet { local, name: _ } => {
            let value = cursor
                .scope()
                .function(super::super::super::super::local::LocalKey::new(
                    super::super::super::super::local::LocalKind::TupleFunction,
                    local.0,
                ));
            Representability::Inhabited(DraftFlow::value(cursor, DraftTupleFunction::new(value)))
        }
        E::Call {
            function,
            args,
        ...
        E::Panic(value) => {
            source_stop(value, cursor, graph, context).map(|flow| flow.map(DraftTupleFunction::new))
        }
        E::BoolCase {
            subject,
            true_,
            ...
            |cursor, graph, context| tuple_function_expr(true_, cursor, graph, context),
            |cursor, graph, context| tuple_function_expr(false_, cursor, graph, context),
            DraftTupleFunction::from_ref,
        ),
        E::IntCase {
            subject,
            clauses,
            ...
            super::super::super::step::steps(steps, cursor, graph, context).and_then(|flow| {
                match flow {
                    ...
                    DraftFlow::Value { cursor, value: () } => {
                        tuple_function_expr(return_, cursor, graph, context)
                    }
                }
            })
        }
