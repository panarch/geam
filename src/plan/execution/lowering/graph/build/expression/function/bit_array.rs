...
pub(in crate::plan::execution::lowering) fn bit_array_function_expr(
...
) -> Representability<DraftFlow<DraftBitArrayFunction>> {
    use super::super::super::instruction::DraftFunctionInstruction as I;
    use module::BitArrayFunctionExprKind as E;
    ...
    match expression.kind() {
        E::Constant(value) => context.bit_array_function_constant(value).map(|id| {
            ...
            DraftFlow::value(cursor, DraftBitArrayFunction::new(value))
        }),
        E::Reference(value) => context
            .bit_array_function_id(value.instantiation())
            .map(|target| {
                reference(
                    shape.clone(),
                ...
                .map(DraftBitArrayFunction::new)
            }),
        E::Closure { function, captures } => {
            context.bit_array_function_id(function).and_then(|target| {
                closure(
                    function,
                    captures,
                    shape.clone(),
                ...
                .map(|flow| flow.map(DraftBitArrayFunction::new))
            })
        }
        E::LocalGet { local, name: _ } => {
            let value = cursor
                ...
                .function(super::super::super::local::LocalKey::new(
                    super::super::super::local::LocalKind::BitArrayFunction,
                    local.0,
                ));
            Representability::Inhabited(DraftFlow::value(cursor, DraftBitArrayFunction::new(value)))
        }
        E::Call {
            function,
            args,
            ...
            DraftFlow::Value {
                ...
                .bit_array_function_function_id(function)
                .map(|function| {
                    let function = execution::function::FunctionFunctionId::BitArray(function);
                    let value = graph.function_instruction(
                        &mut cursor,
                        shape.clone(),
        ...
        E::BoolCase {
            ...
            |cursor, graph, context| bit_array_function_expr(true_, cursor, graph, context),
            |cursor, graph, context| bit_array_function_expr(false_, cursor, graph, context),
            DraftBitArrayFunction::from_ref,
        ),
        E::IntCase {
            subject,
            clauses,
            ...
            |branch, cursor, graph, context| {
                bit_array_function_expr(branch, cursor, graph, context)
            },
            DraftBitArrayFunction::from_ref,
        ),
        E::StringCase {
            ...
            super::super::super::step::steps(steps, cursor, graph, context).and_then(|flow| {
                match flow {
                    ...
                    DraftFlow::Value { cursor, value: () } => {
                        bit_array_function_expr(return_, cursor, graph, context)
                    }
                }
            })
        }
