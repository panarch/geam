...
pub(in crate::plan::execution::lowering) fn string_function_expr(
...
) -> Representability<DraftFlow<DraftStringFunction>> {
    use super::super::super::instruction::DraftFunctionInstruction as I;
    use module::StringFunctionExprKind as E;
    ...
    match expression.kind() {
        E::Constant(value) => context.string_function_constant(value).map(|id| {
            ...
            DraftFlow::value(cursor, DraftStringFunction::new(value))
        }),
        ...
        E::Closure { function, captures } => {
            context.string_function_id(function).and_then(|target| {
                ...
                .map(|flow| flow.map(DraftStringFunction::new))
            })
        }
        E::LocalGet { local, name: _ } => {
            let value = cursor
                ...
                .function(super::super::super::local::LocalKey::new(
                    super::super::super::local::LocalKind::StringFunction,
                    local.0,
                ));
            Representability::Inhabited(DraftFlow::value(cursor, DraftStringFunction::new(value)))
        }
        E::Call {
            function,
            args,
            ...
            DraftFlow::Value {
                ...
                .string_function_function_id(function)
                .map(|function| {
                    let function = execution::function::FunctionFunctionId::String(function);
                    let value = graph.function_instruction(
                        &mut cursor,
                        shape.clone(),
                        I::Call {
        ...
        E::BoolCase {
            ...
            |cursor, graph, context| string_function_expr(true_, cursor, graph, context),
            |cursor, graph, context| string_function_expr(false_, cursor, graph, context),
            DraftStringFunction::from_ref,
        ),
        E::IntCase {
            subject,
            clauses,
            ...
            super::super::super::step::steps(steps, cursor, graph, context).and_then(|flow| {
                match flow {
                    ...
                    DraftFlow::Value { cursor, value: () } => {
                        string_function_expr(return_, cursor, graph, context)
                    }
                }
            })
        }
