...
use crate::gleam_stdlib::dynamic_decode::schema::{
    DecodeError, DecodeErrorConstructor, DecodeListResult, DecodeListResultElements, DecodedItem,
    DictError, DictOk, DictResult, DynamicDict, ItemDecodeLayer, ItemDecoderArguments, PushPath,
};
use crate::{
    HostCall, HostCallCompletion, HostCallError, HostCallable, HostExternal, HostList, HostValue,
};
use ecow::EcoString;
    ...
    Profile: GleamStdlibHostProfile,
{
    let Some(sequence) = dynamic::sequence(&mut call, data) else {
        ...
        let path = call.create_list::<EcoString>([]);
        let error = call.create_custom::<DecodeErrorConstructor>((
            EcoString::from("List"),
            (found, (path, ())),
        ));
        let values = call.create_list::<DecodedItem>([]);
        let errors = call.create_list::<DecodeError>([error]);
        let result = call.create_tuple::<DecodeListResultElements>((values, (errors, ())));
        return Ok(call.return_value(result));
    };
    ...
    while let Some(value) = call.list_item(values, value_index) {
        ...
        if call.list_len(errors) != 0 {
            ...
            while let Some(error) = call.list_item(errors, error_index) {
                ...
                let path = call.create_list::<EcoString>(updated_path);
                updated_errors.push(
                    call.create_custom::<DecodeErrorConstructor>((expected, (found, (path, ())))),
                );
                error_index += 1;
            }
            let empty = call.create_list::<DecodedItem>([]);
            let errors = call.create_list::<DecodeError>(updated_errors);
            let failure = call.create_tuple::<DecodeListResultElements>((empty, (errors, ())));
            return Ok(call.return_value(failure));
        }
        decoded.push(value);
        value_index += 1;
        index += 1;
    ...
    let values = call.create_list::<DecodedItem>(decoded);
    let errors = call.create_list::<DecodeError>([]);
    let result = call.create_tuple::<DecodeListResultElements>((values, (errors, ())));
