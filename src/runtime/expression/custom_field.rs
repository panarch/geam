...
pub(in crate::runtime) fn eval_custom_field(
    ...
    if !access
        .constructors()
        .iter()
        .any(|constructor| *constructor == value.constructor())
    {
        return Err(ExecutionError::CustomFieldDiscriminantMismatch {
            expected_type: plan.custom_value_type(access.source().type_id()),
            expected_constructors: access
                .constructors()
                .iter()
                .map(|constructor| plan.custom_constructor(*constructor).name().clone())
                .collect(),
...
mod tests {
    use crate::plan::{
        ...
        CustomTypePublicity, CustomTypeTemplate, Expr, FunctionExpr, FunctionPlan,
        FunctionReference, FunctionType, IntExpr, IntFunctionId, ListExpr, ModulePlan, ReturnBody,
        ReturnExpr, RuntimeFunctionId, StringExpr, StringFunctionId, TupleExpr, ValueType,
    };
    use crate::plan::{FunctionId, TupleFunctionId};
    use crate::runtime::{
    ...
    fn typed_custom_field_projections_report_every_family_mismatch() {
        ...
        for expected in expected_types {
            ...
            let access = CustomFieldAccess::new(
                CustomExpr::constructor(constructor.clone(), vec![actual_value]),
                0,
                Some("value".into()),
                vec![constructor],
            );
            ...
            let access = CustomFieldAccess::new(
                CustomExpr::constructor(constructor.clone(), vec![outer_actual]),
                0,
                Some("value".into()),
                vec![constructor],
            );
    ...
    fn run_field_projection_module(
        ...
        crate::run_main(&plan).expect_err("direct-mutated custom field should fail")
    }
