...
mod tests {
    ...
    use crate::plan::{
        ...
        ListExpr, ListLocalId, ListValue, NilFunctionExpr, NilFunctionFunctionId, NilFunctionId,
        NilFunctionLocalId, NilFunctionValue, NilLocalId, PanicExpr, PanicSite, ParamLocal,
    ...
    #[test]
    fn eval_nil_function_list_index() {
        let plan = plan();
        let mut frame = Frame::default();
        ...
        assert_eq!(
            eval_nil_function_expr(
                &plan,
                &mut frame,
                &NilFunctionExpr::list_index(list, 0, type_()),
            )
            .expect("expression should evaluate")
            .runtime_id(),
            NilFunctionId(0),
        ...
        assert_eq!(
            eval_nil_function_expr(
                &plan,
                &mut frame,
                &NilFunctionExpr::list_index(list, 1, type_()),
            ),
